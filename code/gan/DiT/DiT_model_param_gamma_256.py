...
def detect_nan(tensor):
    """
    Detects NaN values in a tensor.
    :param tensor: A tensor.
    :return: True if there are NaN values in the tensor, False otherwise.
    """
    return torch.isnan(tensor).any().item()


def deal_nan_img(img):
    temp=torch.ones(img.shape)
    return  temp

def adjust_gamma(gamma_param, img):
    #if detect_nan(img):
    if detect_nan(gamma_param) == True:
        #print("_____________________________")
        gamma_param=gamma_param*0+1

    if detect_nan(img)==True:
        img=img*0+1

    gamma_param = gamma_param.to(img.device).to(img.dtype)
    for _ in range(len(img.shape) - len(gamma_param.shape)):
        gamma_param = torch.unsqueeze(gamma_param, dim=-1)
    img = torch.pow(img + 1e-8, gamma_param+ 1e-8)
...
def calculate_gamma(images):
    gray_images = torch.mean(images, dim=1, keepdim=True)
    mean=torch.mean(gray_images, dim=(1, 2, 3))
    max=torch.max(gray_images, dim=1).values
    max = torch.max(max, dim=1).values
    max = torch.max(max, dim=1).values
    gamma = torch.log(mean) / torch.log(max)
    return gamma
...
class DiffusionModel_Gamma(nn.Module):
    ...
    def forward(self, x, idx):
        ...
        image=adjust_gamma(encode_param,image)
        enocde_param_all=enocde_param_all+encode_param

        encoded_image=image

        batch_brightness = calculate_gamma(image)
        ...
        image = adjust_gamma(decode_param1, image)

        batch_brightness = calculate_gamma(image)
        t = torch.randint(0, diffusion.num_timesteps, (image.shape[0],), device=device)
        decode_param2 = self.decoder2(image, t, batch_brightness)
        image = adjust_gamma(decode_param2, image)

        batch_brightness = calculate_gamma(image)
        t = torch.randint(0, diffusion.num_timesteps, (image.shape[0],), device=device)
        decode_param3 = self.decoder3(image, t, batch_brightness)
        image = adjust_gamma(decode_param3, image)
