...
def adjust_contrast(contrast_param, img):
    contrast_param = contrast_param.to(img.device).to(img.dtype)
    for _ in img.shape[1:]:
        contrast_param = torch.unsqueeze(contrast_param, dim=-1)
    img = img * contrast_param
    return img
...
def calculate_contrast(images):
    ...
    contrast = torch.std(gray_images, dim=(1, 2, 3))
    return contrast
...
class DiffusionModel_Contrast(nn.Module):
    ...
    def forward(self, x, idx):
        ...
        image=adjust_contrast(encode_param,image)
        enocde_param_all=enocde_param_all+encode_param

        encoded_image=image

        batch_brightness = calculate_contrast(image)
        ...
        image = adjust_contrast(decode_param1, image)

        batch_brightness = calculate_contrast(image)
        t = torch.randint(0, diffusion.num_timesteps, (image.shape[0],), device=device)
        decode_param2 = self.decoder2(image, t, batch_brightness)
        image = adjust_contrast(decode_param2, image)

        batch_brightness = calculate_contrast(image)
        t = torch.randint(0, diffusion.num_timesteps, (image.shape[0],), device=device)
        decode_param3 = self.decoder3(image, t, batch_brightness)
        image = adjust_contrast(decode_param3, image)
