...
def adjust_hue(hue_param, img):
    hue_param = hue_param.to(img.device).to(img.dtype)
    for _ in img.shape[1:]:
        hue_param = torch.unsqueeze(hue_param, dim=-1)
    img=img+hue_param
    return img
...
def calculate_hue(images):
    hue=torch.mean(images,dim=(1,2,3))
    return hue
...
class DiffusionModel_Hue(nn.Module):
    ...
    def forward(self, x, idx):
        ...
        image=adjust_hue(encode_param,image)
        enocde_param_all=enocde_param_all+encode_param

        encoded_image=image

        batch_brightness = calculate_hue(image)
        ...
        image = adjust_hue(decode_param1, image)

        batch_brightness = calculate_hue(image)
        t = torch.randint(0, diffusion.num_timesteps, (image.shape[0],), device=device)
        decode_param2 = self.decoder2(image, t, batch_brightness)
        image = adjust_hue(decode_param2, image)

        batch_brightness = calculate_hue(image)
        t = torch.randint(0, diffusion.num_timesteps, (image.shape[0],), device=device)
        decode_param3 = self.decoder3(image, t, batch_brightness)
        image = adjust_hue(decode_param3, image)
